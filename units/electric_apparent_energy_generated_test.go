...
func TestGetElectricApparentEnergyAbbreviation(t *testing.T) {
    tests := []struct {
        name string
        unit units.ElectricApparentEnergyUnits
        want string
    }{
        {
        ...
        {
            name: "invalid unit",
            unit: units.ElectricApparentEnergyUnits("invalid"),
            want: "",
        },
    }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got := units.GetElectricApparentEnergyAbbreviation(tt.unit)
            if got != tt.want {
                t.Errorf("GetElectricApparentEnergyAbbreviation(%v) = %v, want %v", 
                    tt.unit, got, tt.want)
            }
        })
    }
...
func TestElectricApparentEnergy_String(t *testing.T) {
    factory := units.ElectricApparentEnergyFactory{}
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            unit, err := factory.CreateElectricApparentEnergy(tt.value, units.ElectricApparentEnergyVoltampereHour)
            if err != nil {
            ...
            if !strings.HasPrefix(got, tt.want) {
                t.Errorf("ElectricApparentEnergy.String() = %v, want %v", got, tt.want)
            }
        })
    }
}
...
func TestElectricApparentEnergy_BrokenCreation(t *testing.T) {
	// Create a factory instance
	uf := units.ElectricApparentEnergyFactory{}

	_, err := uf.CreateElectricApparentEnergy(100, "unknown")
