...
func TestGetElectricReactiveEnergyAbbreviation(t *testing.T) {
    tests := []struct {
        name string
        unit units.ElectricReactiveEnergyUnits
        want string
    }{
        {
        ...
        {
            name: "invalid unit",
            unit: units.ElectricReactiveEnergyUnits("invalid"),
            want: "",
        },
    }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got := units.GetElectricReactiveEnergyAbbreviation(tt.unit)
            if got != tt.want {
                t.Errorf("GetElectricReactiveEnergyAbbreviation(%v) = %v, want %v", 
                    tt.unit, got, tt.want)
            }
        })
    }
...
func TestElectricReactiveEnergy_String(t *testing.T) {
    factory := units.ElectricReactiveEnergyFactory{}
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            unit, err := factory.CreateElectricReactiveEnergy(tt.value, units.ElectricReactiveEnergyVoltampereReactiveHour)
            if err != nil {
            ...
            if !strings.HasPrefix(got, tt.want) {
                t.Errorf("ElectricReactiveEnergy.String() = %v, want %v", got, tt.want)
            }
        })
    }
}
...
func TestElectricReactiveEnergy_BrokenCreation(t *testing.T) {
	// Create a factory instance
	uf := units.ElectricReactiveEnergyFactory{}

	_, err := uf.CreateElectricReactiveEnergy(100, "unknown")
