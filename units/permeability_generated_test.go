...
func TestGetPermeabilityAbbreviation(t *testing.T) {
    tests := []struct {
        name string
        unit units.PermeabilityUnits
        want string
    }{
        {
            name: "HenryPerMeter abbreviation",
            unit: units.PermeabilityHenryPerMeter,
            want: "H/m",
        },
        {
            name: "invalid unit",
            unit: units.PermeabilityUnits("invalid"),
            want: "",
        },
    }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got := units.GetPermeabilityAbbreviation(tt.unit)
            if got != tt.want {
                t.Errorf("GetPermeabilityAbbreviation(%v) = %v, want %v", 
                    tt.unit, got, tt.want)
            }
        })
    }
...
func TestPermeability_String(t *testing.T) {
    factory := units.PermeabilityFactory{}
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            unit, err := factory.CreatePermeability(tt.value, units.PermeabilityHenryPerMeter)
            if err != nil {
            ...
            if !strings.HasPrefix(got, tt.want) {
                t.Errorf("Permeability.String() = %v, want %v", got, tt.want)
            }
        })
    }
}
