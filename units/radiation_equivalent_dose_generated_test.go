...
func TestRadiationEquivalentDoseConversions(t *testing.T) {
	...
	}
	{
		...
		cacheResult := a.RoentgensEquivalentMan()
		if math.IsNaN(result) || cacheResult != result {
	...
	}
	{
		...
		cacheResult := a.MilliroentgensEquivalentMan()
		if math.IsNaN(result) || cacheResult != result {
...
func TestGetRadiationEquivalentDoseAbbreviation(t *testing.T) {
    tests := []struct {
        name string
        unit units.RadiationEquivalentDoseUnits
        want string
    }{
        {
            name: "Sievert abbreviation",
            unit: units.RadiationEquivalentDoseSievert,
            want: "Sv",
        },
        {
            name: "RoentgenEquivalentMan abbreviation",
            unit: units.RadiationEquivalentDoseRoentgenEquivalentMan,
            want: "rem",
        },
        {
            name: "Nanosievert abbreviation",
            unit: units.RadiationEquivalentDoseNanosievert,
            want: "nSv",
        },
        {
            name: "Microsievert abbreviation",
            unit: units.RadiationEquivalentDoseMicrosievert,
            want: "μSv",
        },
        {
            name: "Millisievert abbreviation",
            unit: units.RadiationEquivalentDoseMillisievert,
            want: "mSv",
        },
        {
            name: "MilliroentgenEquivalentMan abbreviation",
            unit: units.RadiationEquivalentDoseMilliroentgenEquivalentMan,
            want: "mrem",
        },
        {
            name: "invalid unit",
            unit: units.RadiationEquivalentDoseUnits("invalid"),
            want: "",
        },
    }
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got := units.GetRadiationEquivalentDoseAbbreviation(tt.unit)
            if got != tt.want {
                t.Errorf("GetRadiationEquivalentDoseAbbreviation(%v) = %v, want %v", 
                    tt.unit, got, tt.want)
            }
        })
    }
...
func TestRadiationEquivalentDose_String(t *testing.T) {
    factory := units.RadiationEquivalentDoseFactory{}
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            unit, err := factory.CreateRadiationEquivalentDose(tt.value, units.RadiationEquivalentDoseSievert)
            if err != nil {
            ...
            if !strings.HasPrefix(got, tt.want) {
                t.Errorf("RadiationEquivalentDose.String() = %v, want %v", got, tt.want)
            }
        })
    }
}
...
func TestRadiationEquivalentDose_BrokenCreation(t *testing.T) {
	// Create a factory instance
	uf := units.RadiationEquivalentDoseFactory{}

	_, err := uf.CreateRadiationEquivalentDose(100, "unknown")
