...
var internalVitaminAUnitsMap = map[VitaminAUnits]bool{
	
	VitaminAInternationalUnit: true,
}
...
func newVitaminA(value float64, fromUnit VitaminAUnits) (*VitaminA, error) {
	...
	if _, ok := internalVitaminAUnitsMap[fromUnit]; !ok {
		return nil, fmt.Errorf("unknown unit %s in VitaminAUnits", fromUnit)
	}
	a := &VitaminA{}
